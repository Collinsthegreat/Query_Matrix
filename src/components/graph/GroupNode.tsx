...
export function GroupGraphNode({ data, selected }: NodeProps<GraphNodeData>) {
  return (
    <div
      className={cn(
        "min-w-52 rounded-[var(--radius-xl)] border-2 bg-[var(--bg-card)] p-3 text-left shadow-sm transition",
        data.logic === "AND" ? "border-[var(--logic-and)]" : "border-[var(--logic-or)]",
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge tone={data.logic === "AND" ? "accent" : "warning"}>{data.logic}</Badge>
      </div>
      ...
      {data.impactLabel && (
        <div className="mt-2 flex items-center gap-2">
          ...
          <span className="font-mono text-xs font-medium text-[var(--primary)]">{data.impactLabel}</span>
        </div>
      )}
    </div>
  );
