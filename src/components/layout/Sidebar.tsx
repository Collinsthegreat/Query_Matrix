      ...
      <section className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-3">
        ...
        <div className="max-h-44 space-y-2 overflow-auto">
          {entries.length === 0 ? <p className="text-xs text-[var(--text-secondary)]">Run a query to create history.</p> : entries.map((entry) => (
            <div key={entry.id} className="rounded border border-[var(--border)] bg-[var(--bg-input)] p-2 text-xs">
      ...
      <section className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-3">
        ...
        <div className="max-h-44 space-y-2 overflow-auto">
          {presets.length === 0 ? <p className="text-xs text-[var(--text-secondary)]">Save a query to reuse it later.</p> : presets.map((preset) => (
            <div key={preset.id} className="flex items-center gap-2 rounded border border-[var(--border)] bg-[var(--bg-input)] p-2">
              <button type="button" className="min-h-9 flex-1 rounded text-left text-xs font-semibold text-[var(--text-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" onClick={() => importTree(preset.tree)}>
                {preset.name}
              </button>
